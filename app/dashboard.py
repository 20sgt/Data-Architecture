...
def pivot(rows, top_n=None):
    """Long (member, vote_value, n) -> wide dict for st.bar_chart.

    Returns {"member": [...], "<vote value>": [...], ...}. Members are sorted
    by total votes desc and truncated to top_n; vote columns follow VOTE_ORDER
    with unrecognized labels appended alphabetically, so a new Legistar label
    shows up as its own bar instead of vanishing.
    """
    totals, counts = {}, {}
    ...
    members = sorted(totals, key=lambda m: (-totals[m], m))[:top_n]
...
def totals(rows):
    """{vote_value: n} across every member — the tally strip."""
    out = {}
    for _, value, n in rows:
        value = value or "(blank)"
...
def demo():
    ...
    assert pivot(rows, top_n=1)["member"] == ["Dorsey"]
    assert pivot([]) == {"member": []}
